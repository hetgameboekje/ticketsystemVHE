...
- `AI_API_KEY` / `AI_API_URL` / `AI_MODEL` / `AI_CONFIDENCE_DREMPEL` — AI provider for `App\Modules\EmailVerwerking\Services\AiAnalysisService` (defaults target the Anthropic Messages API). Not per-environment-prefixed, same as `APP_ENCRYPTION_KEY`. Empty `AI_API_KEY` fails closed: the analysis endpoint logs the error to `processing_logs` and leaves the e-mail on status `stored` for the next cron run, instead of throwing.
